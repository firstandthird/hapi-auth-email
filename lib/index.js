    ...
    server.route({
      ...
      config: {
        ...
        handler: function(request, reply) {
          ...
          var useJSON = (request.query.type && request.query.type === 'json');
          ...
          callFunc(pluginOptions.getAccount, request, request.payload.email, function(err, user) {
            ...
            valid(user, request.payload.password, function(err, isValid) {
              ...
              if (isValid) {
                request.auth.session.set(user._id);

                callFunc(pluginOptions.onLoginSuccess, request, user);
                server.log(['hapi-auth-email', 'login', 'debug'], user._id);
              ...
              callFunc(pluginOptions.onLoginError, err, 'Incorrect credentials', request, user);
              server.log(['hapi-auth-email', 'login', 'error'], { error: err, message: 'Incorrect credentials' });
    ...
    server.route({
      ...
      config: {
        ...
        handler: function(request, reply) {
          ...
          var useJSON = request.query.type;
          ...
          hash(user, request.payload.password, function(err, hashed) {
            ...
            callFunc(pluginOptions.saveAccount, request, hashed, function(err, savedUser) {
              ...
              request.auth.session.set(savedUser._id);

              callFunc(pluginOptions.onRegisterSuccess, request, savedUser);
              server.log(['hapi-auth-email', 'register', 'debug'], savedUser._id);
