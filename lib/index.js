...
const pluginDefaults = {
  ...
  loginRedirectFilter: (request, uri, done) => {
    done(null, uri);
  }
};
          ...
          callFunc(pluginOptions.getAccount, request, request.payload.email, (err, user) => {
            ...
            valid(user, request.payload.password, (err2, isValid) => {
              ...
              if (isValid) {
                ...
                return callFunc(pluginOptions.loginRedirectFilter, request, request.payload.next || pluginOptions.successEndpont, (error, uri) => {
                  if (error) {
